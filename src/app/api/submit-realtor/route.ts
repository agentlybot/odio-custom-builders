...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { name, brokerage, email, phone, message } = body;
    ...
    await resend.emails.send({
      ...
      subject: `New Realtor Partner Inquiry from ${name}${brokerage ? ` (${brokerage})` : ""}`,
      html: `
        <h2>New Realtor Partner Inquiry</h2>
        <table style="border-collapse: collapse; width: 100%; max-width: 600px;">
          ...
          <tr>
            <td style="padding: 8px; font-weight: bold; border-bottom: 1px solid #eee;">Brokerage</td>
            <td style="padding: 8px; border-bottom: 1px solid #eee;">${brokerage || "Not provided"}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="padding: 8px; border-bottom: 1px solid #eee;">${message || "Not provided"}</td>
          </tr>
        </table>
      `,
    });
    ...
    console.error("Failed to send realtor partner inquiry email:", error);
    return NextResponse.json(
