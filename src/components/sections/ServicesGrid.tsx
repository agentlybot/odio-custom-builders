...
export function ServicesGrid() {
  // Split services: first 3 are renovations (featured), last 2 are repairs
  const renovationServices = SERVICES.slice(0, 3);
  const repairServices = SERVICES.slice(3);
  ...
  return (
    <Section background="white">
      ...
      <div className="space-y-8">
        {/* Top Row - Renovations (Featured, 3-col dark cards) */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {renovationServices.map((service, index) => {
            const labels = ["Most Popular", "Personal Retreat", "Expand Your Home"];
            return (
              <Link
                key={service.slug}
        ...
        {/* Bottom Row - Repairs (2-col white cards) */}
          ...
          {repairServices.map((service, index) => (
            ...
            >
              ...
              <div className="p-6">
                ...
                <h3 className="text-xl font-semibold text-navy-900 mb-2 mt-1 group-hover:text-gold-600 transition-colors">
                  {service.title}
                </h3>
